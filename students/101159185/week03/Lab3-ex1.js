// Ex 1

// ●	Write node script file that use the above two buffers and does the following:
// o	convert buffer_string to a string and output(
// o	convert buffer_string to hex and output
// o	convert buffer_string to utf-8 and read from 0 to 10
// o	join the buffer_array and buffer_string and output
// o	convert the joined_buffer to JSON and output (using Buffer.concat)


const buffer_array = Buffer.from([8,6,7,5,3,0,9]);
const buffer_string = Buffer.from("I'm a string!", "utf-8");

console.log(buffer_array);
console.log(buffer_string);
console.log('#1---------------------------------')
console.log(buffer_string.toString('ascii'));
console.log(buffer_string.toString('hex'));
console.log(buffer_string.toString('utf8',0,10));

joined_buffer = Buffer.concat([buffer_array,buffer_string]);

console.log(joined_buffer);
console.log(joined_buffer.toJSON());